...
function SheetData10() {
  const [data, setData] = useState([]);
  const [topIndex, setTopIndex] = useState(null);

  useEffect(() => {
    const sheetData = sheetsData["Sheet7"];

    if (sheetData && sheetData.length > 0) {
      setData(sheetData);

      const key = "Points For"; // 🔁 Change this to the actual column you're ranking by
